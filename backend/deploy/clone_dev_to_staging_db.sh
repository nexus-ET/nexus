#!/usr/bin/env bash
# Clone Hostinger nexus_edutrust_dev → nexus_edutrust (schema + ALL data),
# grant staging role privileges, point staging .env at Hostinger, restart backend.
#
# Run ON the VPS (both DBs are local — fast):
#   export STAGING_DB_PASSWORD='…'   # nexus_et_admin password (plaintext)
#   sudo -E bash /var/www/nexus/backend/deploy/clone_dev_to_staging_db.sh
#
# Or from your PC (after SSH):
#   scp backend/deploy/clone_dev_to_staging_db.sh root@YOUR_VPS_IP:/tmp/
#   ssh root@YOUR_VPS_IP
#   export STAGING_DB_PASSWORD='…'
#   sudo -E bash /tmp/clone_dev_to_staging_db.sh
#
# Optional overrides:
#   STAGING_DB=nexus_edutrust  STAGING_USER=nexus_et_admin
#   DEV_DB=nexus_edutrust_dev  ENV_FILE=/var/www/nexus/backend/.env
#   DUMP_PATH=/tmp/nexus_edutrust_dev.dump
#
# Does NOT print or write the password into git-tracked files.
...
DUMP_PATH="${DUMP_PATH:-/tmp/${DEV_DB}.$$.dump}"
...
if [[ -z "${STAGING_DB_PASSWORD:-}" ]]; then
  echo "ERROR: set STAGING_DB_PASSWORD to the ${STAGING_USER} plaintext password." >&2
  exit 1
fi

# URL-encode password for DATABASE_URL (! → %21, # → %23, @ → %40, …)
urlencode() {
...
echo "==> 1/7 Privileges for ${STAGING_USER} on ${STAGING_DB}"
...
echo "==> 2/7 Dump ${DEV_DB} (custom format, no-owner/no-acl)"
sudo -u postgres pg_dump -Fc --no-owner --no-acl -d "${DEV_DB}" -f "${DUMP_PATH}"
ls -lh "${DUMP_PATH}"

echo "==> 3/7 Reset public schema on ${STAGING_DB} and restore"
sudo -u postgres psql -v ON_ERROR_STOP=1 -d "${STAGING_DB}" <<SQL
...
GRANT CREATE ON SCHEMA public TO ${STAGING_USER};
SQL
sudo -u postgres pg_restore --no-owner --no-acl -d "${STAGING_DB}" "${DUMP_PATH}" || true
# pg_restore can exit non-zero on benign notices; verify below.

echo "==> 4/7 Re-apply ownership/grants after restore"
# PG16+: identity sequences are linked to their table — ALTER SEQUENCE OWNER fails
# with "Sequence is linked to table …". Own tables/views first (transfers identity
# seq ownership); only ALTER SEQUENCE for non-identity / unlinked sequences.
sudo -u postgres psql -v ON_ERROR_STOP=1 -d "${STAGING_DB}" <<SQL
ALTER DATABASE ${STAGING_DB} OWNER TO ${STAGING_USER};
ALTER SCHEMA public OWNER TO ${STAGING_USER};
GRANT ALL ON SCHEMA public TO ${STAGING_USER};
...
DECLARE r RECORD;
BEGIN
  -- Tables first (identity sequences follow table owner on PG16+)
  FOR r IN
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  -- Sequences not already owned by ${STAGING_USER} and not identity-linked
  FOR r IN
...
# Broad sequence realign (id columns)
sudo -u postgres psql -v ON_ERROR_STOP=1 -d "${STAGING_DB}" <<'SQL'
DO $$
...
echo "==> 5/7 Verify table + alembic parity vs ${DEV_DB}"
...
KEY_TABLES=(users leads students_master institutions programs education_majors countries)
for t in "${KEY_TABLES[@]}"; do
  ...
  echo "  count ${t}: dev=${d} staging=${s}"
done

if [[ "${DEV_TABLES}" != "${STG_TABLES}" ]] || [[ "${DEV_ALEM}" != "${STG_ALEM}" ]]; then
  echo "WARNING: table count or alembic version mismatch — inspect before restart." >&2
fi

echo "==> 6/7 Update ${ENV_FILE} DATABASE_URL (backup first)"
if [[ ! -f "${ENV_FILE}" ]]; then
  echo "ERROR: ${ENV_FILE} missing" >&2
  exit 1
fi
cp -a "${ENV_FILE}" "${ENV_FILE}.bak.$(date +%Y%m%d%H%M%S)"
# Replace DATABASE_URL line(s); keep other keys. Strip CRLF after.
...
# Smoke-test app user can connect
...
echo "==> 7/7 Restart nexus-backend"
...
rm -f "${DUMP_PATH}"
echo ""
echo "Done. Staging DATABASE_URL host form:"
echo "  postgresql+psycopg://${STAGING_USER}:***@127.0.0.1:5432/${STAGING_DB}"
echo "  tables staging=${STG_TABLES} alembic=${STG_ALEM}"
