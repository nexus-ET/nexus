...
**Staging database:** Hostinger KVM 1 PostgreSQL (`nexus_edutrust` / `nexus_et_admin`).  
**Development database:** Hostinger PostgreSQL (`nexus_dev` / `nexus_dev_admin`) — see [setup_dev_db.md](./setup_dev_db.md). Do not reuse staging credentials or Neon still-paper after switching.

| Doc | Purpose |
|-----|---------|
| [setup_dev_db.md](./setup_dev_db.md) | **One-time** develop Hostinger Postgres bootstrap + migrations |
| [setup_staging_db.md](./setup_staging_db.md) | **One-time** staging Hostinger Postgres bootstrap + migrations |
| [DEPLOYMENT_INSTRUCTIONS.md](./DEPLOYMENT_INSTRUCTIONS.md) | Full Hostinger staging deploy |
| [STAGING_CONFIG_REQUIREMENTS.md](./STAGING_CONFIG_REQUIREMENTS.md) | Manual VPS `.env` checklist |
| [STAGING_ENV_CHANGES.md](./STAGING_ENV_CHANGES.md) | Same env notes (shorter) |
| [STAGING_DATABASE_MIGRATIONS.md](./STAGING_DATABASE_MIGRATIONS.md) | Alembic chain reference + empty-DB bootstrap |
| [env.staging.example](./env.staging.example) | Staging `.env` template (no secrets) |

**Database:** `hostinger-staging.sh` runs `bootstrap_alembic.py` → `alembic upgrade head`. Fresh `nexus_edutrust` gets the full schema (head `yy5z6asupermaj`, ~110 public tables).
...
| DB errors | Check `DATABASE_URL` in `.env`; staging must use Hostinger Postgres **`nexus_edutrust`** (see [setup_staging_db.md](./setup_staging_db.md)) |
