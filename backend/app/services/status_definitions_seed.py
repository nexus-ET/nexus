"""Bootstrap status_definitions v2 when the table exists but Alembic seed did not run."""
...
def seed_status_definitions_if_empty(db: Session) -> bool:
    """
    Insert the 39 pipeline stages when status_definitions has no rows.
    ...
    logger.warning("status_definitions is empty — seeding v2 pipeline stages.")
    ...
    db.execute(text("UPDATE status_definitions SET next_stage_id = 10 WHERE id IN (4, 5, 13)"))
    db.execute(text("UPDATE status_definitions SET next_stage_id = 4 WHERE id = 6"))
    db.execute(text("UPDATE status_definitions SET next_stage_id = 16 WHERE id = 12"))
    db.execute(text("UPDATE status_definitions SET next_stage_id = 23 WHERE id = 21"))
    db.execute(text("UPDATE status_definitions SET next_stage_id = 35 WHERE id = 34"))
    db.execute(
        text(
