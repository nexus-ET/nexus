"""reseed status_definitions v3 (45 stages)

Revision ID: s5p8q1r54s0m
Revises: r4n7o2p36q8l
...
revision: str = "s5p8q1r54s0m"
down_revision: Union[str, Sequence[str], None] = "r4n7o2p36q8l"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

# status_definitions v2 (39 rows) → v3 (45 rows)
V2_TO_V3_STATUS_ID_CASE = """
...
def _remap_status_fk_columns() -> None:
    op.execute(
        f"""
        UPDATE leads
...
def _drop_status_definition_fks() -> None:
    op.execute("ALTER TABLE IF EXISTS leads DROP CONSTRAINT IF EXISTS fk_leads_status_definition_id")
    op.execute("ALTER TABLE IF EXISTS leads DROP CONSTRAINT IF EXISTS leads_status_definition_id_fkey")
    op.execute(
    ...
    op.execute(
        ...
        "DROP CONSTRAINT IF EXISTS lead_status_history_status_definition_id_fkey"
    )


def _truncate_status_transitions() -> None:
    op.execute(
        """
        DO $$
        BEGIN
...
def _recreate_status_definitions() -> None:
...
def _reseed_status_transitions() -> None:
    op.execute(
        """
        DO $$
        BEGIN
...
def _restore_status_definition_fks() -> None:
    op.execute(
        """
        ALTER TABLE leads
...
def upgrade() -> None:
    bind = op.get_bind()
    if _is_v3_seeded(bind):
        return

    _remap_status_fk_columns()
    _drop_status_definition_fks()
    _truncate_status_transitions()
    _recreate_status_definitions()
    _reseed_status_transitions()
    _restore_status_definition_fks()
