/** Indian GST place-of-supply states (GSTIN prefix → name). */
...
export function gstStateName(code: string): string {
  ...
  return INDIAN_GST_STATES.find(s => s.code === normalized)?.name || normalized;
}
...
export function stateCodeFromGstin(gstin: string): string {
  ...
  const code = cleaned.slice(0, 2).padStart(2, '0');
  return /^\d{2}$/.test(code) && code !== '00' ? code : '';
}
