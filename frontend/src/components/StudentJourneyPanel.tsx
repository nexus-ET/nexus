...
export default function StudentJourneyPanel({
  ...
  return (
    <>
      ...
      <aside
        className="fixed top-0 right-0 h-full w-full max-w-2xl bg-card border-l border-border-subtle shadow-2xl z-[70] flex flex-col min-h-0"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="shrink-0 flex items-center gap-3 px-5 py-4 border-b border-border-subtle bg-surface-bg/40">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-text-muted">Student Journey</p>
        ...
        <div className="shrink-0 hidden sm:grid grid-cols-[minmax(0,1.4fr)_minmax(110px,0.7fr)_minmax(90px,0.5fr)_minmax(0,1fr)] gap-2 px-5 py-2 border-b border-border-subtle bg-surface-bg/30 text-[10px] font-semibold uppercase tracking-wide text-text-muted">
          <span>Status</span>
          <span>When</span>
          <span>By</span>
          <span>Notes</span>
            ...
            <ul className="divide-y divide-border-subtle">
              {items.map((item, index) => (
                <li
                  ...
                  className={`grid grid-cols-1 sm:grid-cols-[minmax(0,1.4fr)_minmax(110px,0.7fr)_minmax(90px,0.5fr)_minmax(0,1fr)] gap-1 sm:gap-2 px-5 py-2 text-xs hover:bg-surface-bg/50 ${
                    index === items.length - 1 ? 'bg-accent/5' : ''
                  }`}
                >
                  <div className="min-w-0 flex items-start gap-2">
                    <span className="text-[10px] font-mono text-text-muted w-5 shrink-0 pt-0.5">
                      {index + 1}
                    </span>
                    <div className="min-w-0">
                      <span
                        className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[10px] font-semibold leading-tight ${categoryBadgeClass(
                          item.category
                        )}`}
                      >
                        {item.stage_name}
                      ...
                      {item.description ? (
                        <p className="text-[10px] text-text-muted mt-0.5 line-clamp-1">{item.description}</p>
                      ) : null}
                    </div>
                  </div>
                  <div className="sm:pt-0.5 pl-7 sm:pl-0 text-[11px] text-text-muted whitespace-nowrap">
                    {formatTimestamp(item.created_at)}
                  </div>
                  <div className="sm:pt-0.5 pl-7 sm:pl-0 text-[11px] text-text-main truncate">
                    {item.changed_by_label}
                    <span className="text-text-muted">
                      {item.changed_by_type === 'system' ? ' · auto' : ' · admin'}
                    </span>
                  </div>
                  <div className="sm:pt-0.5 pl-7 sm:pl-0 text-[11px] text-text-muted line-clamp-2">
                    {item.comments || '—'}
                  </div>
                </li>
              ))}
